	<section id="sidebar-wrapper">
		<aside>
			<article class="social-icons">
				<h3>Sozial, Info & Kontakt</h3>
				<ul>
					<li id="twitter"><a href="https://twitter.com/JuPisHamburg"><i class="icon-twitter"></i></a></li>
					<li id="facebook"><a href="https://www.facebook.com/jupishamburg"><i class="icon-facebook"></i></a></li>
					<li><a href="https://github.com/jupishamburg"><i class="icon-github"></i></a></li>
					<li><a href="https://wiki.junge-piraten.de/wiki/HH:Hauptseite" style="font-family: serif;font-weight:bold;padding-top:5px;padding-bottom:4px;">W</a></li>
					<li><a href="mailto:info@hamburg.junge-piraten.de"><i class="icon-envelope-alt"></i></a></li>
				</ul>
			</article>

			<?php dynamic_sidebar('sidebar-0'); ?>
		</aside>
	</section>
</div>
<div class="clear"></div>

<?php get_footer(); ?>